...
sintomasClin = st.text_area("Principais sintomas clínicos:")
justificativa = st.text_area("Condições que justificam a internação:")
exames = st.text_area("Resultados de exames realizados:")
diagInicial = st.text_input("Diagnóstico inicial:")
cid = st.text_input("CID:")
cidSecundario = st.text_input("CID secundário (se existir):")
procedimento = st.text_input("Descrição do procedimento:")
cpfMedico = st.text_input("CPF Médico:")
nomeMedico = st.text_input("Nome do Médico solicitante:")
...
if st.button("Gerar Relatório PDF"):
    ...
    if not nomePaciente:
        ...
        canva_texto.drawString(38, 568, f"{municipioUF}")
        ...
        canva_texto.drawString(273, 318, f"{cid}")
